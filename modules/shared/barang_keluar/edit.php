...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi input
    if ($id_barang <= 0 || $jumlah <= 0 || empty($tanggal) || empty($jenis) || $id_user <= 0) {
    ...
    // Update
    $stmt = $koneksi->prepare("UPDATE barang_keluar SET id_barang=?, jumlah=?, tanggal=?, jenis=?, tujuan=?, keterangan=? WHERE id=?");
    $stmt->bind_param("isssssi", $id_barang, $jumlah, $tanggal, $jenis, $tujuan, $keterangan, $id);
