...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id_barang <= 0 || $jumlah_fisik <= 0 || empty($lokasi) || empty($tanggal)) {
        header("Location: edit.php?id=$id&msg=kosong&obj=stok_fisik");
        exit;
    }

    // Ambil stok sistem jika koreksi
    if ($koreksi === 1) {
        $q = "
            SELECT 
    ...
    $stmt = $koneksi->prepare("UPDATE stok_fisik SET id_barang=?, jumlah_fisik=?, stok_sistem=?, koreksi=?, lokasi=?, tanggal=?, keterangan=? WHERE id=?");
    $stmt->bind_param("iiiisssi", $id_barang, $jumlah_fisik, $stok_sistem, $koreksi, $lokasi, $tanggal, $keterangan, $id);
            ...
            <div class="card-body">
                <form method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="lokasi" id="lokasi" class="form-control" required value="<?= htmlspecialchars($data['lokasi']) ?>">
                    </div>
                    <div class="mb-3">
