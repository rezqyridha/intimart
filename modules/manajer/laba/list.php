...
// Mengambil data laba bersih untuk ditampilkan
$query = "
    SELECT lb.id_laba_bersih, lb.id_penjualan, lb.biaya_transportasi, lb.biaya_gaji, lb.total_biaya, lb.bersih, 
           p.nm_brg, p.tgl, p.jumlah, p.hrg_perdus, p.total_penjualan,
           m.nm_manager, m.jabatan
    FROM laba_bersih lb
                                        ...
                                        <table id="pegawai" class="table text-nowrap table-bordered">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Biaya Gaji</th>
                                                    <th>Total Biaya (Transportasi + Gaji)</th>
                                                    <th>Laba Bersih (Pendapatan - Total Biaya)</th>
                                                    <th>Aksi</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                ...
                                                while ($row = $result->fetch_assoc()) {
                                                    // Menambahkan format Rp. pada setiap angka
                                                    $totalPenjualan = number_format($row['total_penjualan'], 0, ',', '.');
                                                    $biayaTransportasi = number_format($row['biaya_transportasi'], 0, ',', '.');
                                                    $biayaGaji = number_format($row['biaya_gaji'], 0, ',', '.');
                                                    $totalBiaya = number_format($row['total_biaya'], 0, ',', '.');
                                                    $labaBersih = number_format($row['bersih'], 0, ',', '.');
                                                    $hrgPerDus = number_format($row['hrg_perdus'], 0, ',', '.'); // Format harga per dus

                                                    echo "<tr>";
                                                    echo "<td>" . $no++ . "</td>"; // Menampilkan nomor urut
                                                    echo "<td>" . $row['nm_manager'] . "</td>"; // Nama Manager
                                                    echo "<td>" . $row['jabatan'] . "</td>"; // Jabatan Manager

                                                    echo "<td>" . date('d-m-Y', strtotime($row['tgl'])) . "</td>"; // Format: 03 Februari 2025

                                                    echo "<td>" . $row['nm_brg'] . "</td>"; // Tanggal Penjualan
                                                    echo "<td>" . $row['jumlah'] . "</td>"; // Jumlah
                                                    echo "<td>Rp. " . $hrgPerDus . "</td>"; // Harga Per Dus dengan format Rp.
                                                    echo "<td>Rp. " . $totalPenjualan . "</td>"; // Total Penjualan
                                                    echo "<td>Rp. " . $biayaTransportasi . "</td>"; // Biaya Transportasi
                                                    echo "<td>Rp. " . $biayaGaji . "</td>"; // Biaya Gaji
                                                    echo "<td>Rp. " . $totalBiaya . "</td>"; // Total Biaya
                                                    echo "<td>Rp. " . $labaBersih . "</td>"; // Laba Bersih
                                                    echo "<td>
                <a href='edit-laba.php?id_laba_bersih=" . $row['id_laba_bersih'] . "' class='btn btn-primary shadow btn-xs sharp mr-1'><i class='fa fa-pencil'></i></a>
                <a href='del-laba.php?id_laba_bersih=" . $row['id_laba_bersih'] . "' class='btn btn-danger shadow btn-xs sharp' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'><i class='fa fa-trash'></i></a>
            </td>";
                                                    echo "</tr>";
                                                }
