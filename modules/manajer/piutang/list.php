...
// Ambil data dari tabel piutang, penjualan, dan manager
$query = "
    SELECT p.id_pelanggan, p.id_penjualan, p.nm_pelanggan, p.nm_toko, p.no_hp, p.alamat, p.foto_ktp, 
           p.tempo_pembayaran, p.nama_barang, p.jumlah_barang, pe.hrg_perdus, 
           (p.jumlah_barang * pe.hrg_perdus) AS total, m.nm_manager, m.jabatan
    FROM piutang p
    LEFT JOIN penjualan pe ON p.id_penjualan = pe.id_penjualan
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
                    <div>
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Halaman Data Piutang Pelanggan</li>
                        </ol>
                    </div>
                </div>
                                        ...
                                        <table id="pegawai" class="table text-nowrap table-bordered">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Jumlah Barang</th>
                                                    <th>Harga Per Dus</th>
                                                    <th>Total (jumlah barang * Harga per Dus)</th> <!-- Kolom Total -->
                                                    <th>Aksi</th>
                                                </tr>
                                            <tbody>
                                                <?php
                                                ...
                                                while ($row = $result->fetch_assoc()) {
                                                    // Format harga per dus dan total dengan format mata uang
                                                    $hrg_perdus = number_format($row['hrg_perdus'], 0, ',', '.');
                                                    $total = number_format($row['total'], 0, ',', '.');

                                                    echo "<tr>";
                                                    echo "<td>{$no}</td>";
                                                    echo "<td>" . htmlspecialchars($row['nm_manager']) . "</td>";  // Menampilkan Nama Manager
                                                    echo "<td>" . htmlspecialchars($row['jabatan']) . "</td>";  // Menampilkan Jabatan Manager

                                                    echo "<td>" . htmlspecialchars($row['nm_pelanggan']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['nm_toko']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['no_hp']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['alamat']) . "</td>";
                                                    echo "<td><a href='../" . htmlspecialchars($row['foto_ktp']) . "' target='_blank'>Lihat File</a></td>";
                                                    echo "<td>" . htmlspecialchars($row['tempo_pembayaran']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['nama_barang']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['jumlah_barang']) . "</td>";
                                                    echo "<td>Rp " . $hrg_perdus . "</td>";  // Menampilkan Harga Per Dus dengan format Rp
                                                    echo "<td>Rp " . $total . "</td>";  // Menampilkan Total dengan format Rp

                                                    // Tombol Edit, Hapus, dan Print
                                                    echo "<td>
            <div class='d-flex align-items-center'>
                <!-- Tombol Edit -->
                <a href='edit-piutang.php?id_pelanggan=" . $row['id_pelanggan'] . "' class='btn btn-primary shadow btn-xs sharp mr-1'>
                    <i class='fa fa-pencil'></i>
                </a>
                <!-- Tombol Hapus -->
                <a href='del-piutang.php?id_pelanggan=" . $row['id_pelanggan'] . "' class='btn btn-danger shadow btn-xs sharp' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'>
                    <i class='fa fa-trash'></i>
                </a>
                <!-- Tombol Print -->
                <a href='javascript:void(0)' onclick='window.print()' class='btn btn-success shadow btn-xs sharp'>
