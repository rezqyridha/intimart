...
// Mengambil data dari tabel stokbrg_periode yang berisi saldo akhir
$query = "
    SELECT s.id_stokperiode, s.id_pesan, s.id_penjualan, s.saldo_akhir, p.jml_brg, j.jumlah
    FROM stokbrg_periode s
    JOIN pesan_barang p ON s.id_pesan = p.id_pesan
    JOIN penjualan j ON s.id_penjualan = j.id_penjualan
";
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
                    <div>
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Halaman Data Stok Masuk & Keluar</li>
                        </ol>
                    </div>
                </div>
                            ...
                            <div class="card custom-card">
                                <div class="card-header justify-content-between">
                                    <div class="card-title">
                                        Data saldo akhir stok masuk dan keluar
                                    </div>
                                </div>
                                <div class="card-body">
                                            ...
                                            <thead>
                                                ...
                                                <?php if ($result->num_rows > 0): ?>
                                                    ...
                                                    <?php while ($row = $result->fetch_assoc()): ?>
                                                        <tr>
                                                            <td><?= $no++; ?></td>
                                                            <td><?= htmlspecialchars($row['jml_brg']); ?></td>
                                                            <td><?= htmlspecialchars($row['jumlah']); ?></td>
                                                            <td><?= htmlspecialchars($row['saldo_akhir']); ?></td>
                                                            <td>
                                                                <div class="d-flex align-items-center">
                                                                    <!-- Tombol Edit -->
                                                                    <a href='edit-stok-barang.php?id_stokperiode=<?= $row['id_stokperiode']; ?>' class='btn btn-primary shadow btn-xs sharp mr-1'>
                                                                        <i class='fa fa-pencil'></i>
                                                                    </a>
                                                                    <!-- Tombol Hapus -->
                                                                    <a href='del-stok-barang.php?id_stokperiode=<?= $row['id_stokperiode']; ?>' class='btn btn-danger shadow btn-xs sharp' onclick='return confirm("Apakah Anda yakin ingin menghapus data ini?")'>
                                                                        <i class='fa fa-trash'></i>
                                                                    </a>
                                                                    <!-- Tombol Print -->
                                                                    <a href='print-stok-barang.php?id_stokperiode=<?= $row['id_stokperiode']; ?>' class='btn btn-info shadow btn-xs sharp'>
