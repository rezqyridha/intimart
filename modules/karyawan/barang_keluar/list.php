...
// Query untuk menampilkan data penjualan beserta data manager (nm_manager dan jabatan)
$query = "SELECT p.*, m.nm_manager, m.jabatan FROM penjualan p LEFT JOIN manager m ON p.id_manager = m.id_manager";
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
                    <div>
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Halaman Data Barang Keluar</li>
                        </ol>
                    </div>
                </div>
                                        ...
                                        <table id="pegawai" class="table text-nowrap table-bordered">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Jumlah</th>
                                                    <th>Harga Per Dus</th>
                                                    <th>Total Penjualan (Jumlah * Harga Per Dus)</th>
                                                    <th>Nama Sales</th>
                                                ...
                                                <?php if ($result->num_rows > 0): ?>
                                                    ...
                                                    <?php while ($row = $result->fetch_assoc()): ?>
                                                        <tr>
                                                            ...
                                                            <td><?php echo $no++; ?></td>
                                                            <td><?php echo $row['nm_manager'] ? $row['nm_manager'] : 'Tidak Ada Manager'; ?></td> <!-- Nama Manager -->
                                                            <td><?php echo $row['jabatan'] ? $row['jabatan'] : 'Tidak Ada Jabatan'; ?></td> <!-- Jabatan Manager -->

                                                            <td><?php echo htmlspecialchars($row['nm_toko']); ?></td>
                                                            <td><?php echo htmlspecialchars(date('d-m-Y', strtotime($row['tgl']))); ?></td>
                                                            <td><?php echo htmlspecialchars($row['no_transaksi']); ?></td>
                                                            <td><?php echo htmlspecialchars($row['nm_brg']); ?></td>
                                                            <td><?php echo htmlspecialchars($row['jumlah']); ?></td>
                                                            <td>Rp <?php echo number_format($row['hrg_perdus'], 0, ',', '.'); ?></td>
                                                            <td>Rp <?php echo number_format($row['total_penjualan'], 0, ',', '.'); ?></td>
                                                            <td><?php echo htmlspecialchars($row['nm_sales']); ?></td>
                                                            <td><?php echo htmlspecialchars($row['area_penjualan']); ?></td>
                                                            <td>
                                                                <div class="d-flex align-items-center">
                                                                    <!-- Tombol Edit -->
                                                                    <a href='edit-barang-keluar.php?id_penjualan=<?php echo $row['id_penjualan']; ?>' class='btn btn-primary shadow btn-xs sharp mr-1'>
                                                                        <i class='fa fa-pencil'></i>
                                                                    </a>
                                                                    <!-- Tombol Hapus -->
                                                                    <a href='del-barang-keluar.php?id_penjualan=<?php echo $row['id_penjualan']; ?>' class='btn btn-danger shadow btn-xs sharp' onclick='return confirm("Apakah Anda yakin ingin menghapus data ini?")'>
                                                                        <i class='fa fa-trash'></i>
                                                                    </a>
                                                                    <!-- Tombol Print -->
                                                                    <a href='print-penjualan.php?id_penjualan=<?php echo $row['id_penjualan']; ?>' class='btn btn-info shadow btn-xs sharp'>
