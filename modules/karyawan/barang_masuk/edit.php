...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $id_masuk = $_POST['id_masuk'];  // Mengambil id_masuk dari form
    $id_pesan = $_POST['id_pesan'];  // Mengambil id_pesan dari form
    $catatan = $_POST['catatan'];    // Mengambil catatan dari form

    // Menyimpan perubahan ke dalam tabel brg_msk
    $stmt = $conn->prepare("UPDATE brg_msk SET catatan = ?, id_pesan = ? WHERE id_masuk = ?");
    $stmt->bind_param("sii", $catatan, $id_pesan, $id_masuk);

    if ($stmt->execute()) {
        echo "<script>alert('Data berhasil diperbarui.'); window.location.href='data_barang_masuk.php';</script>";
    } else {
        // Debugging jika query gagal
        echo "<script>alert('Terjadi kesalahan: " . $stmt->error . "'); window.location.href='edit-barang-masuk.php?id_masuk=" . $id_masuk . "';</script>";
    }
...
if (isset($_GET['id_masuk'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM brg_msk WHERE id_masuk = ?");
    ...
    echo "<script>alert('ID Masuk tidak ditemukan!'); window.location.href='data_barang_masuk.php';</script>";
}

// Mengambil data id_pesan dari tabel pesan_barang untuk dropdown
$query = "SELECT id_pesan, nm_brg FROM pesan_barang";
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
                    <div>
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Halaman Edit Barang Masuk</li>
                        </ol>
                    </div>
                </div>
                                        ...
                                        <form action="" method="POST">
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 col-xl-6 col-xxl-6 mb-3">
                                                    <label for="id_pesan" class="form-label">Pilih Barang</label>
                                                    <select name="id_pesan" id="id_pesan" class="form-control" required>
                                                        <?php foreach ($barangList as $barangOption): ?>
                                                            <option value="<?php echo $barangOption['id_pesan']; ?>"
                                                                <?php echo $barang['id_pesan'] == $barangOption['id_pesan'] ? 'selected' : ''; ?>>
                                                                <?php echo htmlspecialchars($barangOption['nm_brg']); ?>
                                                            </option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                </div>
                                                ...
                                                <div class="col-md-6 col-xl-6 col-xxl-6 mb-3">
                                                    ...
                                                    <textarea name="catatan" id="catatan" rows="4" class="form-control" required><?php echo htmlspecialchars($barang['catatan']); ?></textarea>
                                                </div>
                                            </div>

                                            <!-- Input tersembunyi untuk id_masuk -->
                                            <input type="hidden" name="id_masuk" value="<?php echo $barang['id_masuk']; ?>">

                                            <!-- Submit Button -->
                                            <div class="row">
                                                <div class="col-12">
                                                    <button type="submit" class="btn btn-primary">Perbarui</button>
                                                    <a href="data_barang_masuk.php" class="btn btn-secondary">Batal</a>
                                                </div>
                                            </div>
                                        </form>
