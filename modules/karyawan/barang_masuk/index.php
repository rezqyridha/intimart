...
// Mengambil data dari tabel brg_msk berdasarkan id_pesan
$query = "SELECT 
            brg_msk.id_masuk, 
            brg_msk.catatan, 
            pesan_barang.kode_barang, 
            pesan_barang.nm_brg, 
            pesan_barang.jml_brg, 
            pesan_barang.hrg_perunit, 
            pesan_barang.total_hrg, 
            pesan_barang.tgl_pengiriman_inginkan,
            manager.nm_manager,
            manager.jabatan
          FROM 
            brg_msk
          JOIN 
            pesan_barang ON brg_msk.id_pesan = pesan_barang.id_pesan
          JOIN
            manager ON pesan_barang.id_manager = manager.id_manager";
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
                    <div>
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Halaman Data Barang Masuk</li>
                        </ol>
                    </div>
                </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered">
                                            <thead>
                                                <tr>
                                                    <th>No</th>
                                                    <th>Nama Manager</th> <!-- Kolom untuk Manager -->
                                            ...
                                            <tbody>
                                                <?php
                                                $no = 1; // Menetapkan nomor urut mulai dari 1
                                                foreach ($barangMasukList as $barangMasuk): ?>
                                                    <tr>
                                                        <td><?php echo $no++; ?></td> <!-- Penomoran otomatis -->
                                                        <td><?php echo htmlspecialchars($barangMasuk['nm_manager']); ?></td> <!-- Menampilkan Nama Manager -->
                                                        <td><?php echo htmlspecialchars($barangMasuk['jabatan']); ?></td> <!-- Menampilkan Jabatan -->

                                                        <td><?php echo htmlspecialchars($barangMasuk['kode_barang']); ?></td>
                                                        <td><?php echo htmlspecialchars($barangMasuk['nm_brg']); ?></td>
                                                        <td><?php echo htmlspecialchars($barangMasuk['jml_brg']); ?></td>
                                                        <td><?php echo number_format($barangMasuk['hrg_perunit'], 0, ',', '.'); ?></td>
                                                        <td><?php echo number_format($barangMasuk['total_hrg'], 0, ',', '.'); ?></td>
                                                        <td><?php echo htmlspecialchars(date('d-m-Y', strtotime($barangMasuk['tgl_pengiriman_inginkan']))); ?></td>
                                                        <td><?php echo htmlspecialchars($barangMasuk['catatan']); ?></td>
                                                        <td>
                                                            <div class="d-flex align-items-center">
                                                                <!-- Tombol Edit -->
                                                                <a href='edit-barang-masuk.php?id_masuk=<?php echo $barangMasuk['id_masuk']; ?>' class='btn btn-primary shadow btn-xs sharp mr-1'>
                                                                    <i class='fa fa-pencil'></i>
                                                                </a>
                                                                <!-- Tombol Hapus -->
                                                                <a href='del-barang-masuk.php?id_masuk=<?php echo $barangMasuk['id_masuk']; ?>' class='btn btn-danger shadow btn-xs sharp' onclick='return confirm("Apakah Anda yakin ingin menghapus data ini?")'>
                                                                    <i class='fa fa-trash'></i>
                                                                </a>
                                                                <!-- Tombol Print -->
                                                                <a href='print-barang-masuk.php?id_masuk=<?php echo $barangMasuk['id_masuk']; ?>' class='btn btn-info shadow btn-xs sharp'>
