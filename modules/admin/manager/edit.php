...
// Mendapatkan ID manager dari parameter URL
$id_manager = isset($_GET['id_manager']) ? intval($_GET['id_manager']) : 0;

// Ambil data manager berdasarkan ID
$queryManager = "SELECT * FROM manager WHERE id_manager = ?";
$stmtManager = $conn->prepare($queryManager);
$stmtManager->bind_param("i", $id_manager);
$stmtManager->execute();
...
if (!$dataManager) {
    echo "<script>alert('Data tidak ditemukan!'); window.location.href='data_manager.php';</script>";
    exit;
}

// Menangani penyimpanan perubahan
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $nm_manager = $_POST['nm_manager'];
    $jabatan = $_POST['jabatan'];

    // Update data manager
    $updateQuery = "UPDATE manager SET nm_manager = ?, jabatan = ? WHERE id_manager = ?";
    $updateStmt = $conn->prepare($updateQuery);
    $updateStmt->bind_param("ssi", $nm_manager, $jabatan, $id_manager);

    if ($updateStmt->execute()) {
        echo "<script>alert('Data berhasil diperbarui.'); window.location.href='data_manager.php';</script>";
    } else {
        echo "<script>alert('Terjadi kesalahan: " . $updateStmt->error . "'); window.location.href='edit_manager.php?id=$id_manager';</script>";
    }
    $updateStmt->close();
}
$conn->close();
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
                    <div>
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Halaman Edit Manager</li>
                        </ol>
                    </div>
                </div>
                                ...
                                <div class="card-body">
                                    <div class="row gy-4">
                                        ...
                                        <form action="" method="POST">
                                            <div class="row">
                                                <div class="mb-3">
                                                    <label for="nm_manager" class="form-label">Nama Manager</label>
                                                    <input type="text" class="form-control" name="nm_manager" value="<?php echo htmlspecialchars($dataManager['nm_manager']); ?>" required>
                                                </div>
                                                <div class="mb-3">
                                                    <label for="jabatan" class="form-label">Jabatan</label>
                                                    <input type="text" class="form-control" name="jabatan" value="<?php echo htmlspecialchars($dataManager['jabatan']); ?>" required>
                                                </div>
                                            </div>
                                            ...
                                            <div class="row">
                                                <div class="col-12">
                                                    ...
                                                    <a href="data_manager.php" class="btn btn-secondary">Batal</a>
                                                </div>
                                            </div>
                                        </form>
