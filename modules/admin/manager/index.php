...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO manager (nm_manager, jabatan) VALUES (?, ?)");
    $stmt->bind_param("ss", $nm_manager, $jabatan);

    if ($stmt->execute()) {
        echo "<script>alert('Data berhasil disimpan.'); window.location.href='data_manager.php';</script>";
    } else {
        echo "<script>alert('Terjadi kesalahan: " . $stmt->error . "'); window.location.href='manager.php';</script>";
    }
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
                    <div>
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Halaman Data Manager</li>
                        </ol>
                    </div>
                </div>
                                            ...
                                            <tbody>
                                                ...
                                                if ($result->num_rows > 0) {
                                                    ...
                                                    while ($row = $result->fetch_assoc()) {
                                                        ...
                                                        echo "<td>
                <a href='edit-manager.php?id_manager=" . $row['id_manager'] . "' class='btn btn-primary shadow btn-xs sharp mr-1'><i class='fa fa-pencil'></i></a>
                <a href='del-manager.php?id_manager=" . $row['id_manager'] . "' class='btn btn-danger shadow btn-xs sharp' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'><i class='fa fa-trash'></i></a>
            </td>";
                                                        echo "</tr>";
                                                    }
                                                } else {
