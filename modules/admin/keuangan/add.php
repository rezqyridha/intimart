...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $id_penjualan = $_POST['id_penjualan']; // ID penjualan
    $id_laba_bersih = $_POST['id_laba_bersih']; // ID laba bersih
    $id_manager = $_POST['id_manager']; // ID manager

    // Query untuk mendapatkan data penjualan terkait dengan id_penjualan
    $queryPenjualan = "
    SELECT p.nm_brg, p.total_penjualan
    FROM penjualan p
    ...
    // Query untuk mendapatkan data laba bersih terkait dengan id_laba_bersih
    $queryLabaBersih = "
    SELECT lb.total_biaya
    FROM laba_bersih lb
    ...
    if ($rowPenjualan = $resultPenjualan->fetch_assoc()) {
        if ($rowLabaBersih = $resultLabaBersih->fetch_assoc()) {
            // Menghitung saldo kas berdasarkan total_penjualan dan total_biaya
            $total_penjualan = $rowPenjualan['total_penjualan'];
            $total_biaya = $rowLabaBersih['total_biaya'];
            $saldo_kas = $total_penjualan - $total_biaya; // Hasil saldo kas

            // Menyimpan data ke dalam tabel arus_kas
            $insertStmt = $conn->prepare("INSERT INTO arus_kas (id_arus_kas, id_penjualan, id_laba_bersih, id_manager, saldo_kas) VALUES (?, ?, ?, ?, ?)");
            $insertStmt->bind_param("iiiii", $id_arus_kas, $id_penjualan, $id_laba_bersih, $id_manager, $saldo_kas);

            if ($insertStmt->execute()) {
                echo "<script>alert('Data berhasil disimpan.'); window.location.href='data_arus_kas.php';</script>";
            } else {
                echo "<script>alert('Terjadi kesalahan: " . $insertStmt->error . "'); window.location.href='arus_kas.php';</script>";
            }

            $insertStmt->close();
        } else {
            echo "<script>alert('Data laba bersih tidak ditemukan.'); window.location.href='arus_kas.php';</script>";
        }
    } else {
        echo "<script>alert('Data penjualan tidak ditemukan.'); window.location.href='arus_kas.php';</script>";
    }
...
// Mengambil data penjualan untuk dropdown
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
                    <div>
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Halaman Form Arus Kas</li>
                        </ol>
                    </div>
                </div>
                                        ...
                                        <form action="" method="POST">
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 col-xl-6 col-xxl-6 mb-3">
                                                    ...
                                                    <select class="form-control" name="id_manager" required>
                                                        <option value="">Pilih Manager</option>
                                                        <?php foreach ($managerList as $manager): ?>
                                                            <option value="<?php echo $manager['id_manager']; ?>">
                                                                <?php echo $manager['nm_manager'] . " - " . $manager['jabatan']; ?>
                                                            </option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                </div>
                                                ...
                                                <div class="col-md-6 col-xl-6 col-xxl-6 mb-3">
                                                    ...
                                                    <select class="form-control" name="id_penjualan" required>
                                                        ...
                                                        <?php foreach ($penjualanList as $penjualan): ?>
                                                            <option value="<?php echo $penjualan['id_penjualan']; ?>" <?php echo isset($_POST['id_penjualan']) && $_POST['id_penjualan'] == $penjualan['id_penjualan'] ? 'selected' : ''; ?>>
                                                ...
                                                <div class="col-md-6 col-xl-6 col-xxl-6 mb-3">
                                                    ...
                                                    <select class="form-control" name="id_laba_bersih" required>
                                                        ...
                                                        <?php foreach ($labaBersihList as $labaBersih): ?>
                                                            <option value="<?php echo $labaBersih['id_laba_bersih']; ?>">
