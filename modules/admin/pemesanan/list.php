...
$query = "SELECT p.id_pesan, m.nm_manager, m.jabatan, p.tgl_pemesanan, p.no_pemesanan, p.nm_pengaju, p.nm_supplier, p.alamat_pengiriman, p.kode_barang, p.nm_brg, p.jml_brg, p.hrg_perunit, p.total_hrg, p.tgl_pengiriman_inginkan FROM pesan_barang p JOIN manager m ON p.id_manager = m.id_manager";
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
                    <div>
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Halaman Data Pemesanan</li>
                        </ol>
                    </div>
                </div>
                                        ...
                                        <table id="pegawai" class="table text-nowrap table-bordered">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Harga Per Dus</th>
                                                    <th>Total Harga</th>
                                                    <th>Tanggal Pengiriman yang Diinginkan</th>
                                                    <th>Aksi</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                ...
                                                if ($result->num_rows > 0) {
                                                    ...
                                                    while ($row = $result->fetch_assoc()) {
                                                        echo "<tr>";
                                                        echo "<th>" . $no++ . "</th>";
                                                        echo "<td>" . htmlspecialchars($row['nm_manager']) . "</td>";
                                                        echo "<td>" . htmlspecialchars($row['jabatan']) . "</td>";
                                                        echo "<td>" . htmlspecialchars($row['tgl_pemesanan']) . "</td>";
                                                        echo "<td>" . htmlspecialchars($row['no_pemesanan']) . "</td>";
                                                        echo "<td>" . htmlspecialchars($row['nm_pengaju']) . "</td>";
                                                        echo "<td>" . htmlspecialchars($row['nm_supplier']) . "</td>";
                                                        ...
                                                        echo "<td>" . htmlspecialchars($row['jml_brg']) . "</td>";
                                                        echo "<td>Rp " . number_format($row['hrg_perunit'], 0, ',', '.') . "</td>";
                                                        echo "<td>Rp " . number_format($row['total_hrg'], 0, ',', '.') . "</td>";
                                                        echo "<td>" . htmlspecialchars($row['tgl_pengiriman_inginkan']) . "</td>";
                                                        echo "<td>
                            <a href='edit-pemesanan.php?id_pesan=" . $row['id_pesan'] . "' class='btn btn-primary shadow btn-xs sharp mr-1'>
                                <i class='fa fa-pencil'></i>
                            </a>
                            <a href='del-pemesanan.php?id_pesan=" . $row['id_pesan'] . "' class='btn btn-danger shadow btn-xs sharp' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'>
                                <i class='fa fa-trash'></i>
                                                    ...
                                                    echo "<tr>";
                                                    echo "<td colspan='13'>Tidak ada data ditemukan</td>";
                                                    echo "</tr>";
                                                }
                                                ?>
                                            </tbody>
